...
fls_l8 = list.files("/mnt/CEPH_PROJECTS/AI4EBV/EO/HLS/32TPS", 
                    ...
                    recursive = T, 
                    full.names = TRUE)
list_l8 = tibble(pth = fls_l8, 
                 base_name = basename(fls_l8), 
                 year_doy = gsub(".*T32TPS\\.(20[0-2][0-9][0-3][0-9][0-9])\\.v1\\.4.*", "\\1", fls_l8))
list_l8 = list_l8 %>% 
...
list_l8 = list_l8 %>% arrange(date)

sum(duplicated(list_l8$year_doy)) # this shouldn't be at all i guess
nrow(list_l8)
range(list_l8$date)
...
path_out = "/mnt/CEPH_PROJECTS/ECO4Alps/Forest_Disturbances/01_data/03_hls_l30_v014_ndvi"
aoi = area_bbox
...
# apply in parallel
a = Sys.time()
fls_ndvi = parLapply(cl = cl, X = purrr::transpose(list_l8), fun = function(x){
  ...
  gdal_translate(file_in = paste0("HDF4_EOS:EOS_GRID:'", x$pth, "':Grid:band05"), 
